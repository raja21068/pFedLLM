...
def build_server_llm(cfg) -> ServerLLM:
    llm = ServerLLM(
        embed_dim=cfg.embedding_dim,       # 1024
        output_dim=cfg.embedding_dim,      # 1024
    ...
    # ── Load MLRG checkpoint if provided ──────────────────────────
    ckpt_path = getattr(cfg, 'mlrg_ckpt_path', None)
    if ckpt_path:
        print(f"[ServerLLM] Loading MLRG checkpoint from {ckpt_path}")
        ckpt  = torch.load(ckpt_path, map_location='cpu')
        state = ckpt.get('state_dict', ckpt)

        # MLRG hidden_size=768, pFedLLM embed_dim=1024
        # Add projection layer to bridge the gap
        llm.mlrg_proj = nn.Linear(cfg.embedding_dim, 768)   # 1024 → 768

        # Load text encoder weights from MLRG
        txt_weights = {
        ...
        if txt_weights:
            llm.text_encoder.load_state_dict(txt_weights, strict=False)
            print(f"[ServerLLM] Loaded text encoder ({len(txt_weights)} keys)")

        # Load cross-modal fusion weights from MLRG
        fusion_weights = {
            k.replace('cross_modal_fusion.', ''): v
            for k, v in state.items()
            if k.startswith('cross_modal_fusion.')
        }
        if fusion_weights:
            llm.fusion.load_state_dict(fusion_weights, strict=False)
            print(f"[ServerLLM] Loaded fusion layer ({len(fusion_weights)} keys)")

        # Freeze everything
        llm.freeze_parameters()
        print("[ServerLLM] MLRG weights loaded and frozen.")
